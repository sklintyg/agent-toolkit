...
7. `outline: none` in CSS with no replacement focus style
8. `tabIndex={0}` on `<div>` or `<span>`
9. Hardcoded `#hex` / `rgb()` color values
10. `<img>` without `alt`
11. Related inputs (checkbox/radio groups) not in `<fieldset>` + `<legend>`; errors not linked via `aria-describedby`
12. WCAG 2.2: sticky/fixed elements that could fully obscure a focused component (2.4.11); touch targets < 24×24px (2.5.8); `aria-label` that replaces rather than includes visible text (2.5.3)
13. Component-specific findings from Step 2
...
- **High [Design]** — WCAG A/AA issue that requires a design decision or redesign rather than a code-only fix; log as a Jira ticket for the design team
...
Total: {n} High, {n} High [Design], {n} Medium, {n} Low

### Design-level actions (log as Jira tickets)
...
- If an IDS component lacks the needed ARIA prop, flag it as `[IDS limitation]` in the findings table. Record the component name and the specific missing prop or behavior. Suggest a native HTML wrapper with the required ARIA attributes alongside the IDS CSS classes as a workaround. Prompt the developer to report the gap to the IDS team so it can be fixed upstream.
